...
namespace TarodevController
{
    ...
    public class PlayerController : MonoBehaviour, IPlayerController
    {
        ...
        protected virtual void GatherInput()
        {
            ...
            if (_stats.SnapInput)
            {
                FrameInput.Move.x = Mathf.Abs(FrameInput.Move.x) < _stats.HorizontalDeadzoneThreshold
                    ? 0
                    : Mathf.Sign(FrameInput.Move.x);
                FrameInput.Move.y = Mathf.Abs(FrameInput.Move.y) < _stats.VerticalDeadzoneThreshold
                    ? 0
        ...
        protected virtual void CheckCollisions()
        {
            ...
            _wallHitCount =
                Physics2D.OverlapBoxNonAlloc(bounds.center, bounds.size, 0, _wallHits, _stats.ClimbableLayer);

            _hittingWall = Physics2D.CapsuleCast(_col.bounds.center, _col.size, _col.direction, 0,
                new Vector2(_input.FrameInput.Move.x, 0), _stats.GrounderDistance, ~_stats.PlayerLayer);

            Physics2D.queriesHitTriggers = true; // Ladders are set to Trigger
            _ladderHitCount =
                Physics2D.OverlapBoxNonAlloc(bounds.center, bounds.size, 0, _ladderHits, _stats.LadderLayer);
        ...
        {
            Physics2D.queriesHitTriggers = false;
            var hit = Physics2D.OverlapCapsule(pos + col.offset, col.size - _skinWidth, col.direction, 0,
                ~_stats.PlayerLayer);
            Physics2D.queriesHitTriggers = _cachedTriggerSetting;
            return !hit;
        }
        ...
        protected virtual void HandleWalls()
        {
            ...
            _currentWallJumpMoveMultiplier =
                Mathf.MoveTowards(_currentWallJumpMoveMultiplier, 1f, 1f / _stats.WallJumpInputLossFrames);
            ...
            bool ShouldStickToWall()
            {
                ...
                return !_stats.RequireInputPush ||
                       (HorizontalInputPressed && Mathf.Sign(FrameInput.Move.x) == WallDirection);
            }
        }
        ...
        protected virtual bool LedgeClimbInputDetected =>
        ...
        protected virtual bool TryGetLedgeCorner(out Vector2 cornerPos)
        {
            ...
            var hit3 = Physics2D.Raycast(grabHeight + new Vector2(WallDirection * 0.5f, _stats.LedgeRaycastSpacing),
        ...
        {
            transform.position =
                _rb.position = _ledgeCornerPos + Vector2.Scale(_stats.StandUpOffset, new(WallDirection, 1f));
        ...
        protected virtual bool CanEnterLadder =>
            _ladderHitCount > 0 && _fixedFrame > _frameLeftLadder + _stats.LadderCooldownFrames;

        protected virtual bool ShouldMountLadder => _stats.AutoAttachToLadders ||
                                                    FrameInput.Move.y > _stats.VerticalDeadzoneThreshold ||
                                                    (!_grounded && FrameInput.Move.y <
                                                        -_stats.VerticalDeadzoneThreshold);

        protected virtual bool ShouldDismountLadder => !_stats.AutoAttachToLadders && _grounded &&
                                                       FrameInput.Move.y < -_stats.VerticalDeadzoneThreshold;
        ...
        protected virtual void HandleLadders()
        {
            ...
            if (ClimbingLadder && ShouldCenterOnLadder)
            {
                ...
                _rb.position = Vector2.SmoothDamp(pos, new Vector2(targetX, pos.y), ref _ladderSnapVel,
        ...
        protected virtual bool CanWallJump => (_isOnWall && !_isLeavingWall) ||
                                              (_wallJumpCoyoteUsable &&
                                               _fixedFrame < _frameLeftWall + _stats.WallJumpCoyoteFrames);

        protected virtual bool CanAirJump => !_grounded && _airJumpsRemaining > 0;

        protected virtual void HandleJump()
        {
            if (!_endedJumpEarly && !_grounded && !FrameInput.JumpHeld && _rb.velocity.y > 0)
        ...
        protected virtual void NormalJump()
        {
            ...
            _frameJumpWasPressed =
                0; // prevents double-dipping 1 input's jumpToConsume and buffered jump for low ceilings
        ...
        protected virtual bool HorizontalInputPressed =>
            Mathf.Abs(FrameInput.Move.x) > _stats.HorizontalDeadzoneThreshold;
        ...
        protected virtual void HandleHorizontal()
        {
            ...
            {
                var crouchPoint =
                    Mathf.InverseLerp(0, _stats.CrouchSlowdownFrames, _fixedFrame - _frameStartedCrouching);
                var diminishedMaxSpeed = _stats.MaxSpeed * Mathf.Lerp(1, _stats.CrouchSpeedPenalty, crouchPoint);
                _speed.x = Mathf.MoveTowards(_speed.x, FrameInput.Move.x * diminishedMaxSpeed,
        ...
        protected virtual void HandleVertical()
        {
            ...
            else if (_grounded && _speed.y <= 0f)
            {
                ...
                if (TryGetGroundNormal(out var groundNormal)
                    //NOTE:
                    // Climbable을 왼쪽면으로 올라가서 도착했을때 groundNormal이 right방향이 되어
                    // 이후 speed 계산식이 영향을 받아 이후 left 입력 시 위로 튀는 현상 방지
                    && Mathf.RoundToInt(Vector2.Dot(Vector2.up, groundNormal)) != 0)
                {
        ...
        protected virtual void ApplyMovement()
        {
            ...
            _currentExternalVelocity = Vector2.MoveTowards(_currentExternalVelocity, Vector2.zero,
        ...
        private void OnDrawGizmos()
        {
            ...
            if (_stats.AllowLedges && _stats.ShowLedgeDetection)
            {
                ...
                Gizmos.DrawRay(grabHeight + _stats.LedgeRaycastSpacing * Vector3.down,
        ...
        private void OnValidate()
        {
            ...
            if (_standingCollider == null)
                Debug.LogWarning("Please assign a Capsule Collider to the Standing Collider slot", this);
            if (_crouchingCollider == null)
                Debug.LogWarning("Please assign a Capsule Collider to the Crouching Collider slot", this);
